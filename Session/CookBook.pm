...
C<CGI::Session::CookBook> is a tutorial that accompanies B<CGI::Session> distribution. It shows the usage of the library in web applications and demonstrates practical solutions for certain problems. We do not recommend you to read this tutorial unless you're familiar with L<CGI::Session|CGI::Session> and it's syntax.
...
Although we are using default B<DSN> in our examples, you feel free to use any configuration you please.
...
The above syntax will get "first_name" and "email" parameters from the B<CGI.pm> and saves them to the B<CGI::Session> object.Now some other time or even in some other place we can simply say
...
=head1 REMEMBER THE REFERRER

=head2 PROBLEM

You run an outsourcing service, and people get referred to your program from other sites. After finishing the process, which might take several click-troughs, you need to provide them with a link which takes them to a site where they came from. In other words, after 10 clicks through your pages you need to recall the referred link, which takes the user to your site.

=head2 SOLUTION

This solution is similar to the previous one, but instead of getting the data from the submitted form, you get it from HTTP_REFERRER environmental variable, which holds the link to the referred page. But you should be cautious, because the click on your own page to the same application generates a referral as well, in this case with your own link. So you need to watch out for that by saving the link only if it doesn't already exist. This approach is suitable for the application which ALWAYS get accessed by clicking links and posting forms, but NOT by typing in the url. Good examples would be voting polls, shopping carts among many others.

    $ENV{HTTP_REFERRER} or die "Illegal use";

    unless ( $session->param("referrer") ) {
        $session->param("referrer", $ENV{HTTP_REFERRER});
    }

In the above code, we simply save the referrer in the session under the "referrer" parameter. Note, that we first check if it was previously saved, in which case there would be no need to override it. It also means, if the referrer was not saved previously, it's most likely the first visit to the page, and the HTTP_REFERRER holds the link to the link we're interested in, not our own.

When we need to present the link back to the referred site, we just do:

    $href = $session->param("referrer");
...
and this will print the list in nicely formatted table. For more information on associating an object with the B<HTML::Template> refer to L<HTML::Template manual|HTML::Template>
...
Again, the exact implementation of the site will depend on the implementation of this solution. This example is pretty much similar to the way we implemented the browsing history in the previous example. But instead of saving the links of the pages, we simply save the ProductID as the arrayref in the session parameter called, say, "CART". In the following example we tried to represent the imaginary database in the form of a hash.
...
C<cmd> CGI parameter is a run mode for the application, in this particular example it's "add", which tells the application that an item is about to be added. C<itemID> tells the application which item should be added. You might as well go with the item title, instead of numbers, but most of the time in dynamically generated sites you prefer itemIDs over their titles, since titles tend to be not consistent (it's from experience):
...
As you see, things are quite straight-forward this time as well. We're accepting three arguments, getting the itemID from the C<itemID> CGI parameter, retrieving contents of the current cart from the "CART" session parameter, updating the contents with the information we know about the item with the C<itemID>, and storing the modified $cart back to "CART" session parameter. When done, we simply display the cart. If anything doesn't make sense to you, STOP! Read it over!

Here are the contents for C<display_cart()>, which simply gets the shopping cart's contents from the session parameter and generates a list:
...
A slight problem in the above template: TOTAL_PRICE doesn't exist. To fix this problem we need to introduce a slight modification to our C<add_item()>, where we also save the pre calculated total price in the "total_price" session parameter. Try it yourself.
