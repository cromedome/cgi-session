...
unlink('./cgisess_symlink_session','./cgisess_symlink_session_link');
ok(my $s = CGI::Session->new('driver:file;id:static','symlink_session',{Directory=>'.'}),'Create new session named symlink');
...
ok(my $ns = CGI::Session->new('driver:file;id:static','symlink_session_link',{Directory=>'.'}), 'Get our symlinked session');
ok(! -l $ns,'we should have wiped out the symlink');
isnt($ns->param('passthru'),1,'this session should be unique');

unlink('./cgisess_symlink_session_link');
